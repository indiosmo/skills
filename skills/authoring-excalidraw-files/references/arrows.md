...
Complete guide for creating elbow arrows with proper connections in architecture diagrams.

---

## Elbow Arrow Properties
...
| Ellipse | Left | `(x, y + height/2)` |
| Ellipse | Right | `(x + width, y + height/2)` |
...
### L-Shape (Fan-out from Orchestrator)
...
When N arrows leave from same edge, spread evenly across 20%-80% of edge:
...
When using bindings, add arrow reference to shape:
...
| Arrow Type | Label Position |
|------------|----------------|
| Vertical | `x = arrow.x - 20`, `y = arrow.y + height/2` |
| Horizontal | `x = arrow.x + width/2`, `y = arrow.y - 20` |
| L-shaped | At corner or longest segment midpoint |

---

## Width/Height Calculation

Arrow `width` and `height` = bounding box of points (absolute values):
...
| Mistake | Symptom | Fix |
|---------|---------|-----|
| Missing `elbowed: true` | Curved arrows | Add all three elbow properties |
| Arrow x,y at shape center | Floating arrows | Calculate from edge point |
| Wrong width/height | Clipped arrows | Use bounding box of points |
| Same start position for all | Overlapping arrows | Use staggering formula |
| Negative offset without abs | Wrong bounding box | width/height are always positive |
